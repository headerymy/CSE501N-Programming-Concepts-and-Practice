...
public class ListItem {
	...
	public ListItem duplicate() {
		ListItem dup = new ListItem(this.number, null);
		if (this.next != null)
			dup = new ListItem(this.number, this.next.duplicate());
		return dup; // FIXME
	}
	...
	public ListItem stretch(int n) {
		ListItem pointer = new ListItem (this.number, this.next); 
		ListItem head = new ListItem (this.number, this.next);
		ListItem pointer2 = head;
		
		while (pointer!=null){ 
			int i = 0;
			while(i<n) {
				pointer2.next= new ListItem(pointer.number, pointer.next);
				pointer2 = pointer2.next;
				i++;
			}
			pointer = pointer.next;
		}
		return head.next;  // FIXME
	}
	...
	public static ListItem evenElements(ListItem ls) {
		if (ls == null) return null;
			...
			return ListItem.evenElements(ls.next); // FIXME
	}	
